...
| Thuật toán | Ở đâu | Vai trò |
|---|---|---|
| **TF-IDF + Cosine similarity** | `ranking/TfIdfScorer.java` | $\mathrm{tf} = 1 + \log_{10} f$, $\mathrm{idf} = \log_{10}(N/\mathrm{df})$, điểm $= \dfrac{\vec{q}\cdot\vec{d}}{\lVert\vec{q}\rVert\,\lVert\vec{d}\rVert}$, xấp xỉ $\lVert\vec{d}\rVert \approx \sqrt{\lvert D\rvert}$ (kiểu Lucene classic Similarity). |
| **BM25 (Okapi BM25)** | `ranking/BM25Scorer.java` | Baseline chuẩn công nghiệp: $\sum_q \mathrm{IDF}(q)\dfrac{f(q,D)(k_1+1)}{f(q,D)+k_1\left(1-b+b\frac{\lvert D\rvert}{\mathrm{avgdl}}\right)}$ với $k_1=1{,}2$, $b=0{,}75$. Hai ưu điểm so với TF-IDF: **bão hoà tần suất** và **chuẩn hoá độ dài có tham số điều chỉnh**. IDF theo Robertson–Sparck Jones nên không bao giờ âm. |
| **PageRank (power iteration)** | `ranking/PageRankService.java` | $PR(j) = \dfrac{1-d}{N} + d\left(\sum_{i \to j}\dfrac{PR(i)}{L(i)} + \dfrac{M_{\text{dangling}}}{N}\right)$, $d = 0{,}85$, lặp tới khi hội tụ ($L_1 < 10^{-6}$) hoặc tối đa 100 vòng; nhân ma trận-vector qua `SparseMatrix.multiply` ($O(\mathrm{nnz})$/vòng); xử lý riêng dangling node. |
...
| **nDCG@k** | `eval/EvaluationMetrics.java` | $DCG@k = \sum_{i=1}^{k}\dfrac{2^{rel_i}-1}{\log_2(i+1)}$, chuẩn hoá bằng $IDCG@k$ của thứ tự lý tưởng. Dùng độ lợi **hàm mũ** để nhấn mạnh tài liệu "rất liên quan" gấp 3 lần "liên quan" (thay vì 2:1 nếu dùng tuyến tính). |
